...
function getGooglePhotosImage_(albumId, clientID) {
  try {
    ...
    try {
      // Use Drive API to list files in the album/folder
      ...
      if (imageFiles.length > 0) {
        // Select image based on day of year (cycling through all images)
        const imageIndex = dayOfYear % imageFiles.length;
        const selectedImage = imageFiles[imageIndex];
        
        console.log(`Selected Google Photos image ${imageIndex + 1}/${imageFiles.length}: ${selectedImage.name}`);
        
        return {
          url: selectedImage.url,
          caption: `📸 ${selectedImage.caption} (${imageIndex + 1}/${imageFiles.length})`
        };
      }
