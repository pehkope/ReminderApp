...
public class ApiService
{
    ...
    public async Task<ApiResult<ReminderApiResponse>> GetDataAsync(string? clientId = null, int? maxRetries = null, int? timeoutOverrideSeconds = null, bool fast = false)
    {
        ...
        for (int attempt = 1; attempt <= actualMaxRetries; attempt++)
        {
            try
            {
                ...
                var baseUrl = string.IsNullOrEmpty(_apiSettings.BaseUrl) ? "https://script.google.com/macros/s/AKfycbx0ROcZNK38kSmajzzuOzlGmt4yDGkkhSE4-0qcpeDYbReLDs6IptJw7y7xB5LgVq0AmQ/exec" : _apiSettings.BaseUrl;
    ...
    public async Task<bool> AcknowledgeTaskAsync(string taskType, string timeOfDay, string description = "")
    {
        ...
        {
            // Fallback for Azure deployment if config loading fails
            var baseUrl = string.IsNullOrEmpty(_apiSettings.BaseUrl) ? "https://script.google.com/macros/s/AKfycbweTEKmqomRkyzy9mEs_9QE2zLhWW2ZEKlSeiSTKlmj7SQeK6yllNn3Xzy-htpa160/exec" : _apiSettings.BaseUrl;
